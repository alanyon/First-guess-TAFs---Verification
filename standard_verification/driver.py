...
def main_from_params(start_dt, end_dt, sitelist, ver_period,
    ...
    """
    Run the verification for a single set of parameters, without going
    through the command line argument parser.

    This allows the driver to be called directly from another script
    (e.g. the rolling verification workflow) rather than as a subprocess.

    Args:
        start_dt (datetime): start date and time
        end_dt (datetime): end date and time
        sitelist (list): list containing the station ICAO
        ver_period (timedelta): verification period (TAF length)
        verpy_vis_out (str): NetCDF filename for visibility output
        verpy_clb_out (str): NetCDF filename for cloud base output
    ...
    # Replicate the config handling done in get_arguments()
    config = configparser.ConfigParser()
