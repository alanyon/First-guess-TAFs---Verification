...
def do_t_tests(sp_stats, param):
    ...
    pt_stats = {'Category': [], 'XGBoost T-Statistic': [],
    ...
    for cat in range(1, NUM_CATS[param] + 1):
        ...
        # Add to stats dictionary
        pt_stats['Category'].append(f'Category {cat}')
        pt_stats['XGBoost T-Statistic'].append(old_xg_t)
        pt_stats['XGBoost P-Value'].append(old_xg_p)
        pt_stats['Random Forest T-Statistic'].append(old_rf_t)
        pt_stats['Random Forest P-value'].append(old_rf_p)

    # Create dataframe from stats
    stats_df = pd.DataFrame(pt_stats)

    # Set the Category column as the index so we can display it nicely
    stats_df.set_index('Category', inplace=True)
    ...
    ax.xaxis.tick_top()
    ax.set_title(f'{PARAMS[param].title()} Peirce Skill Scores T-Tests\n',
